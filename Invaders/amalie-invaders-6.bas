...
'bullits bookeeping
const bullits = 10
dim integer bullits_ready = 1
dim integer bullits_status(bullits)
dim float bullits_xpos(bullits)
dim float bullits_xvelocity(bullits)
dim float bullits_ypos(bullits)
dim float bullits_yvelocity(bullits)
...
do
  ...
  s=keydown(0)
  for l=1 to s
    text 700,0,str$(keydown(l))
    if keydown(l)=130 then
      xpos=xpos-1
    elseif keydown(l)=131 then
      xpos=xpos+1
    elseif keydown(l)=115 then
      save image "temp.jpg"
    elseif keydown(l)=32 and bullits_ready = 1 then
      fire_bullits xpos+4*5, 580
    endif
  next l
...
sub render_bullits
  ...
  for a=1 to bullits
    if bullits_status(a) = 1 then
      sprite write a, bullits_xpos(a), bullits_ypos(a)
      count = count + 1
      bullits_xpos(a) = bullits_xpos(a) + bullits_xvelocity(a)
      bullits_ypos(a) = bullits_ypos(a) + bullits_yvelocity(a)
      if bullits_xpos(a) < 0 or bullits_xpos(a)>799 or bullits_ypos(a) < 0 then
        bullits_status(a) = 0
      end if
    end if
  next a
  bullits_ready = (count = 0)
end sub

sub fire_bullits xpos, ypos
  local integer a
  for a=1 to bullits
    bullits_status(a) = 1
    bullits_xpos(a) = xpos
    bullits_ypos(a) = ypos
    bullits_xvelocity(a) = 0.5 - rnd
    bullits_yvelocity(a) =  -(3*rnd+4)
  next a
  bullits_ready = 0
end sub
