  ...
  do
    for a= 0 to 301
      ...
      rotate2 = rotate + 5*delta_rotate
      for b = 0 to 59
        xp%(b) = xoffset+xpos(b,a)* cos(rotate2) + ypos(b,a)* sin(rotate2)
        yp%(b) = yoffset-(ypos(b,a)* cos(rotate2) - xpos(b,a)* sin(rotate2))
      next b
      polygon n%, xp%(), yp%(), rgb(black), rgb(black)
